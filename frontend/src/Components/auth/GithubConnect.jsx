...
export default function GitHubConnect() {
  ...
  useEffect(() => {
    // Check for OAuth callback result in URL first
    const params = new URLSearchParams(window.location.search);
    const githubParam = params.get("github");

    if (githubParam === "connected") {
      // Clean up URL then refresh status
  ...
  const checkGitHubStatus = async () => {
    try {
      ...
      // Backend returns: { github: { connected, username, scopes } }
      setIsGitHubConnected(user?.github?.connected ?? false);
      setGithubUsername(user?.github?.username ?? null);
      setError(null);
    } catch (err) {
  ...
  const connectGitHub = async () => {
    try {
      setConnecting(true);
      setError(null);
      // Fetches OAuth URL from backend (with Bearer token), then redirects to GitHub
